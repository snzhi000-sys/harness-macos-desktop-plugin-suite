...
配置保留规则只有一个受管例外：已启用的字面量 File Edit 条目缺少 `shellTransactionLifecycle` 时补入 `true`。显式值和禁用条目不会被覆盖。备份与原子 patch 替换保障可恢复性，标识相同的启动也执行检查。仅更新模块无法启用旧配置中缺少开关的功能。候选发布验证包内迁移和真实 Bash 审核恢复，而非仅验证干净安装。
