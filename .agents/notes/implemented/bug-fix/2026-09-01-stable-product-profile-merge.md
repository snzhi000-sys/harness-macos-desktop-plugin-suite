...
The configuration-preservation rule has one managed exception: enabled literal File Edit entries missing `shellTransactionLifecycle` receive `true`. Explicit values and disabled entries are not overridden. A backup and atomic patch replacement preserve recoverability, including on same-identifier launches. Merely updating modules cannot enable a feature whose legacy configuration omits its opt-in. Candidate publication verifies the packaged migration and real Bash audit/recovery, not only clean installation.
