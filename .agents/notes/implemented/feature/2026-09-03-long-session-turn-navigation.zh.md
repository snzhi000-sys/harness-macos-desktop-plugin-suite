...
普通历史请求每页读取十条追加来源消息，显式跳转保留每批 250 条。两种操作都通过 `historyError` 发布可重试失败，不将失败标记为历史耗尽，也不丢弃已有窗口。重连使旧 generation 的响应和清理失效；重新打开常驻 Session 保留已加载页面。非末页空响应属于错误，不触发自动重试。Host 消息分组和完整冷日志校验继续作为权威。
