...
- Submission publishes the temporary text/image echo synchronously, then yields one browser task before encoding and calling the Host. It does not wait for an animation frame, because backgrounded or suspended documents may not receive one. A failed send restores the complete text and image draft without clobbering text or images added while the request was in flight. Removal, successful send, session-scope disposal, rendered-history disposal, and application disposal revoke the object URLs they own.
...
Pi-AI and the hand-written DeepSeek adapter are certified visual-input routes. Both resolve `ctx.attachments` at request time, recursively preserve durable image references including references nested inside tool results, and emit native image content only for exact models that declare image input. The DeepSeek route prefers Files API references and scopes cached file ids by content-addressed attachment, normalized endpoint, and a one-way credential digest. Controlled upload failure falls back to bounded inline data. An explicitly stale file id invalidates only the matching mapping, triggers one re-upload and one chat retry, and never enters an adapter retry loop. Request-time service resolution keeps Cordis load order from freezing optional attachment availability. No adapter may flatten or skip an image.
...
| `packages/llm/llm-deepseek` | Resolve durable images into scoped Files API references or bounded inline data while preserving reasoning replay. |
...
- Client unit tests cover paste and drop, mixed clipboard text, image-only send, submission when animation frames are suspended, draft restoration, ordering, draft/session-scope/application object-URL cleanup, and a deferred historical read that completes after disposal; the keyless assembled built-client lane (`apps/web/tests/image-display.snapshot.ts`, `DSH_EXAMPLE_MODE=lib pnpm run test:snapshot`) covers the historical user and assistant galleries over the authorized attachment route, the original-size lightbox, and the composer paste rail.
- Adapter and compaction tests cover native Pi-AI and DeepSeek image conversion, scoped Files API reuse, stale-id re-upload, controlled inline fallback, cancellation-isolated concurrent upload, late attachment-service composition, text-only rejection, recursively nested tool-result images, preserved summary input, and explicit image-output rejection.
...
- DeepSeek file ids are provider state rather than durable Session content. Endpoint/credential scoping and one-shot stale-id recovery prevent cross-account reuse and retry loops, while inline fallback spends more request bytes when Files API is unavailable.
