...
- 提交会同步发布临时图文回显，然后让出一个浏览器任务，再开始编码并调用宿主。该流程不等待动画帧，因为后台或暂停状态的文档可能不会收到动画帧。发送失败时恢复完整的文本与图片草稿，但不会覆盖请求飞行期间新增的文本或图片。移除、发送成功、会话 scope 释放、已渲染历史记录释放和应用释放都会撤销各自持有的对象 URL。
...
Pi-AI 与手写 DeepSeek 适配器都是经过认证的视觉输入路径。两者都在请求时解析 `ctx.attachments`，递归保留每个持久图片引用，包括嵌套在工具结果中的引用，并且仅为明确声明支持图片输入的模型生成提供方原生图片内容。DeepSeek 路径优先使用 Files API 引用，并按内容寻址附件、规范化端点和单向凭据摘要限定文件 id 缓存。受控上传故障会降级为有界内联数据。明确过期的文件 id 只会使匹配映射失效，触发一次重新上传和一次 chat 重试，绝不会进入适配器重试循环。在请求时解析服务，可避免 Cordis 加载顺序将可选附件服务的可用性固化。任何适配器都不得将图片展平或跳过。
...
| `packages/llm/llm-deepseek` | 将持久图片解析为限定作用域的 Files API 引用或有界内联数据，同时保留推理回放。 |
...
- 客户端单元测试覆盖粘贴与拖放、混合剪贴板文本、仅图片发送、动画帧暂停时仍能提交、草稿恢复、顺序、草稿、会话作用域和应用层级的对象 URL 清理，以及一项在释放后才完成的延迟历史读取；keyless 的组装后构建产物通道（`apps/web/tests/image-display.snapshot.ts`，`DSH_EXAMPLE_MODE=lib pnpm run test:snapshot`）覆盖经授权附件路由渲染的历史用户与助手图片画廊、原图 lightbox，以及 composer 粘贴缩略图条。
- 适配器与压缩测试覆盖 Pi-AI 和 DeepSeek 原生图片转换、限定作用域的 Files API 复用、过期 id 重新上传、受控内联降级、并发上传的取消隔离、后置附件服务组合、仅文本拒绝、递归嵌套在工具结果中的图片、保留摘要输入，以及明确拒绝图片输出。
...
- DeepSeek 文件 id 属于提供方状态，而不是持久 Session 内容。端点／凭据作用域与一次性过期 id 恢复可以防止跨账号复用和重试循环；Files API 不可用时，内联降级会消耗更多请求字节。
