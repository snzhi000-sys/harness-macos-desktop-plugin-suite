...
test('Stable adds the bundled pet to an old composition and preserves user configuration', async () => {
  const paths = fixture()
  await install(paths, { channel: 'stable' })
  const profile = join(paths.userData, 'harness/profiles/web')
  const before = { name: 'user-profile', dependencies: { custom: '1' }, dsh: { profile: { bundles: ['custom'] } }, custom: true }
  writeFileSync(join(profile, 'package.json'), JSON.stringify(before))
  writeFileSync(join(paths.resourcesPath, 'profile-bootstrap/profile-id'), '2222222222222222')
  await installBundledProfile({ ...paths, channel: 'stable', isPackaged: true, extractArchive: async (_archive, destination) => {
    mkdirSync(join(destination, 'node_modules/dsh-desktop-pet'), { recursive: true })
    writeFileSync(join(destination, 'package.json'), JSON.stringify({ dependencies: { 'dsh-desktop-pet': '*' }, dsh: { profile: { bundles: ['dsh-desktop-pet'] } } }))
  } })
  const after = JSON.parse(readFileSync(join(profile, 'package.json'), 'utf8'))
  assert.deepEqual(after, { ...before, dependencies: { custom: '1', 'dsh-desktop-pet': '*' }, dsh: { profile: { bundles: ['custom', 'dsh-desktop-pet'] } } })
  assert.deepEqual(JSON.parse(readFileSync(join(profile, 'package.json.before-desktop-pet'), 'utf8')), before)
  assert.equal(await installBundledProfile({ ...paths, channel: 'stable', isPackaged: true }), false)
})
...
test('Stable upgrade repairs missing Shell transaction config and preserves user settings', async () => {
  const paths = fixture()
  await install(paths, { channel: 'stable', profileId: '1111111111111111' })
  const profile = join(paths.userData, 'harness', 'profiles', 'web')
  const patch = join(profile, 'cordis.patch.yml')
  const original = '# user comment\n- id: personal\n  config:\n    value: !!js process.platform\n- insert:\n    - id: dsh-file-edit\n      name: dsh-file-edit\n      config:\n        userOption: kept\n'
  writeFileSync(patch, original)
  await install(paths, { channel: 'stable', profileId: '2222222222222222' })
  const migrated = readFileSync(patch, 'utf8')
  assert.match(migrated, /shellTransactionLifecycle: true/)
  assert.match(migrated, /userOption: kept/)
  assert.match(migrated, /!!js process.platform/)
  assert.match(migrated, /# user comment/)
  assert.equal(readFileSync(patch + '.before-shell-transactions', 'utf8'), original)
  assert.equal(await install(paths, { channel: 'stable' }), false)
  assert.equal(readFileSync(patch, 'utf8'), migrated)
})

test('matching Profile ID still repairs legacy insert without config', async () => {
  const paths = fixture()
  await install(paths, { channel: 'stable' })
  const patch = join(paths.userData, 'harness', 'profiles', 'web', 'cordis.patch.yml')
  writeFileSync(patch, '- insert:\n    - id: dsh-file-edit\n      name: dsh-file-edit\n')
  assert.equal(await install(paths, { channel: 'stable' }), true)
  assert.match(readFileSync(patch, 'utf8'), /shellTransactionLifecycle: true/)
})

test('migration preserves explicit false and disabled plugins and rejects invalid YAML', async () => {
  const paths = fixture()
  await install(paths, { channel: 'stable' })
  const patch = join(paths.userData, 'harness', 'profiles', 'web', 'cordis.patch.yml')
  for (const original of [
    '- id: dsh-file-edit\n  config: { shellTransactionLifecycle: false }\n',
    '- id: dsh-file-edit\n  disabled: true\n',
  ]) {
    writeFileSync(patch, original)
    assert.equal(await install(paths, { channel: 'stable' }), false)
  ...
  await assert.rejects(install(paths, { channel: 'stable', profileId: '2222222222222222' }), /invalid Profile YAML/)
  assert.equal(readFileSync(patch, 'utf8'), invalid)
  assert.equal(readFileSync(join(paths.userData, 'harness', 'profiles', '.web-bundled-profile-id'), 'utf8').trim(), '0123456789abcdef')
})
