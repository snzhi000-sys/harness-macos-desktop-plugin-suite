...
分享构建还会包含一份已安装 Web 插件的干净快照。首次启动且不存在 `profiles/web` 目录时，桌面壳会在启动 Harness 前安装该快照；已有 profile 永远不会被替换。快照只包含插件包和经过清理的组成清单，不包含会话记录、工作区、凭据、设置、应用日志、状态存储或本机专属路径。
...
该命令会创建采用临时签名的应用，并将其复制到 `/Applications/DeepSeek Harness.app`。如果已有安装，会将其保留为 `/Applications/DeepSeek Harness.previous.app`；如果这个备份已经存在，安装脚本会拒绝覆盖。
