...
/** 归一化（按选项）后比较两行是否相等。 */
function lineEqual(a, b, options) {
...
export function diffLines(before, after, options = {}) {
    ...
    if (n === 0)
        return after.map(l => ({ type: 'insert', text: l.text }));
    if (m === 0)
        return before.map(l => ({ type: 'delete', text: l.text }));
    // 1. 公共前缀
    let pre = 0;
    while (pre < n && pre < m && lineEqual(before[pre], after[pre], options))
        pre++;
    // 2. 公共后缀
    let suf = 0;
    while (suf < n - pre && suf < m - pre && lineEqual(before[n - 1 - suf], after[m - 1 - suf], options))
        suf++;
    const np = n - pre - suf;
    const mp = m - pre - suf;
    let middle = [];
    if (np === 0 && mp === 0) {
        ...
        middle = after.slice(pre, m - suf).map(l => ({ type: 'insert', text: l.text }));
    }
    else if (mp === 0) {
        middle = before.slice(pre, n - suf).map(l => ({ type: 'delete', text: l.text }));
    }
    else if (np + mp > NP_MP_MAX) {
        // 规模超限：回退
        middle = [
            ...before.slice(pre, n - suf).map(l => ({ type: 'delete', text: l.text })),
            ...after.slice(pre, m - suf).map(l => ({ type: 'insert', text: l.text })),
        ];
    }
    else {
        ...
        const set = new Set();
        for (let i = 0; i < mp; i++)
            set.add(normKey(after[pre + i].text, options));
        let common = false;
        for (let i = 0; i < np; i++) {
            if (set.has(normKey(before[pre + i].text, options))) {
                common = true;
                break;
            }
        }
    ...
    const ops = [];
    for (let i = 0; i < pre; i++)
        ops.push({ type: 'equal', text: before[i].text });
    ops.push(...middle);
...
function myersCore(before, after, options) {
    const n = before.length;
    const m = after.length;
    const max = n + m;
    const offset = max;
    ...
    if (!found) {
        ...
        return [
            ...before.map(l => ({ type: 'delete', text: l.text })),
            ...after.map(l => ({ type: 'insert', text: l.text })),
        ];
    }
    // 回溯 trace 构建操作序列
    const ops = [];
 ...
 */
export function effectiveOps(ops, beforeTrailing, afterTrailing) {
...
export function renderUnified(beforeLabel, afterLabel, before, after, ops, context, beforeTrailing, afterTrailing) {
    if (ops.every(o => o.type === 'equal') && beforeTrailing === afterTrailing)
        return '';
    ...
    // 按 context 合并 hunk
    const changeIdx = [];
    work.forEach((o, i) => { if (o.type !== 'equal')
        changeIdx.push(i); });
    const hunks = [];
...
export function unifiedDiff(beforeText, afterText, options = {}) {
