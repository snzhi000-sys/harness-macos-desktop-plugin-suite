...
function scalarEqual(a, b, ignoreCase) {
...
function childPath(path, keyOrIndex) {
...
function diffPair(a, b, path, out, options, depth) {
    ...
    if (typeof a === 'string' && typeof b === 'string' && options.ignoreCase) {
        if (normalizeString(a, options.ignoreCase) === normalizeString(b, options.ignoreCase))
            return;
    }
    if (Array.isArray(a) && Array.isArray(b)) {
        ...
        for (let i = 0; i < len; i++) {
            ...
            if (i >= a.length)
                out.push({ path: childPath(path, i), kind: 'add', after: pb });
            else if (i >= b.length)
                out.push({ path: childPath(path, i), kind: 'remove', before: pa });
            else
                diffPair(pa, pb, childPath(path, i), out, options, depth + 1);
        }
        return;
    }
    if (isPlainObject(a) && isPlainObject(b)) {
        ...
        for (const k of keyList) {
            ...
            if (!hasA)
                out.push({ path: childPath_, kind: 'add', after: b[k] });
            else if (!hasB)
                out.push({ path: childPath_, kind: 'remove', before: a[k] });
            else
                diffPair(a[k], b[k], childPath_, out, options, depth + 1);
        }
        return;
    }
    // 类型不同或标量不等
...
export function jsonDiff(beforeJson, afterJson, options = {}) {
    const out = [];
...
export function scanDuplicateKeys(text) {
    ...
    for (let i = 0; i < text.length; i++) {
        ...
        if (inString) {
            ...
            if (ch === '"') {
                ...
                if (expectingKey && top()?.isObj) {
                    const t = top();
                    if (t.keys.has(buf) && !dups.includes(buf))
                        dups.push(buf);
                    t.keys.add(buf);
                }
                expectingKey = false;
        ...
        if (ch === '{') {
            stack.push({ isObj: true, keys: new Set() });
            expectingKey = true;
            continue;
        }
        if (ch === '[') {
            stack.push({ isObj: false, keys: new Set() });
            expectingKey = false;
            continue;
        }
        if (ch === '}' || ch === ']') {
...
export function jsonDiffText(beforeText, afterText, options = {}) {
    ...
    try {
        ...
        throw new Error(`diff: before is not valid JSON (${e.message})`);
    }
    try {
        afterParsed = JSON.parse(afterText);
    }
    catch (e) {
        throw new Error(`diff: after is not valid JSON (${e.message})`);
    }
