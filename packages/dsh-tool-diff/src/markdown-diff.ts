...
import { splitLines, diffLines, renderUnified, lineStats, type Line } from './text-diff.ts'
...
export function tokenizeMarkdown(text: string): MdBlock[] {
  ...
  while (i < lines.length) {
    ...
    if (raw.trim() === '') { blocks.push({ kind: 'blank', line: lineNo, text: '' }); i++; continue }
    ...
    if (fence) {
      ...
      while (j < lines.length) {
        ...
        if (close && close[1]![0] === marker && close[1]!.length >= fenceLen) { end = j; break }
        j++
      }
    ...
    if (QUOTE_RE.test(raw)) {
      ...
      while (j < lines.length && (QUOTE_RE.test(lines[j]!.text) || lines[j]!.text.trim() === '')) {
        if (lines[j]!.text.trim() === '') { j++; continue }
    ...
    if (LIST_RE.test(raw) || ORDERED_RE.test(raw)) {
      ...
      while (j < lines.length) {
        ...
        if (t.trim() === '') break
        if (LIST_RE.test(t) || ORDERED_RE.test(t)) { blockLines.push(t); j++; continue }
        if (/^\s+/.test(t)) { blockLines.push(t); j++; continue }
        break
      }
    ...
    if (HR_RE.test(raw)) { blocks.push({ kind: 'hr', line: lineNo, text: raw }); i++; continue }

    if (HTML_COMMENT_RE.test(raw)) {
      const blockLines = [raw]
      let j = i + 1
      while (j < lines.length && !lines[j]!.text.includes('-->')) { blockLines.push(lines[j]!.text); j++ }
      if (j < lines.length) blockLines.push(lines[j]!.text)
...
function buildPaths(blocks: MdBlock[]): Map<MdBlock, string> {
  const map = new Map<MdBlock, string>()
  let currentHeadingPath = ''
  const headingCounters = new Map<string, number>()
  let lastHeading: MdBlock | null = null
  let blockCounter = 0
...
function kindToken(kind: MdBlock['kind']): string {
  switch (kind) {
...
function blockEqual(a: MdBlock, b: MdBlock, options: MdDiffOptions): boolean {
  ...
  if (options.ignoreCase && a.text.toLowerCase() === b.text.toLowerCase()) return true
...
export function markdownDiff(beforeText: string, afterText: string, options: MdDiffOptions = {}): MdReport {
  ...
  const headingChanges: MdHeadingChange[] = []
  const blockChanges: MdBlockChange[] = []
  const codeBlockChanges: MdCodeBlockChange[] = []
  ...
  const parentPath = (h: MdBlock) => {
    const p = h.path!
    const idx = p.lastIndexOf(' > ')
    return idx === -1 ? '' : p.slice(0, idx)
  }
  const afterParentMap = new Map<string, MdBlock[]>()
  ...
  const lb: Line[] = nb.map(b => ({ text: kindToken(b.kind), hash: 0 }))
  const la: Line[] = na.map(b => ({ text: kindToken(b.kind), hash: 0 }))
  for (const l of lb) l.hash = hashStr(l.text)
  for (const l of la) l.hash = hashStr(l.text)
