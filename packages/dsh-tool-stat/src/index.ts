...
import { describeValues, percentileValues } from './describe.ts'
import { frequencyValues } from './frequency.ts'
import { correlationValues } from './correlation.ts'
 ...
 */
export function runStatAction(args: unknown): StatResult {
  ...
  switch (action) {
    ...
    case 'correlation':
      return correlationValues(values, other as number[], method)
  }
}
...
export function apply(ctx: Context): void {
  ctx.tools.register(defineTool({
    name: 'stat',
    description:
      'Deterministic statistics over explicit finite numeric arrays. ' +
    ...
    execute: (args) => {
      ...
      return Promise.resolve(runStatAction(args) as unknown as JsonValue)
    },
    timeoutMs: 2000,
  }))
}
