...
export function percentileAt(sorted, p) {
    ...
    if (n === 0)
        throw new Error('stat: percentile requires at least 1 value');
    if (p < 0 || p > 1)
        throw new Error('stat: percentile must be within 0..100');
...
function assertFinite(fields, what) {
...
export function describeValues(values, sample) {
...
export function percentileValues(values, percentiles) {
    const sorted = sortedCopy(values);
