...
| `audit_root` | string | Exposed when `auditFullAccessWrites` is enabled. Under Full Access, an absolute existing directory anywhere on disk containing every path this foreground call may change; defaults to `workdir`. Reads remain unrestricted and writes outside this root are denied. |
