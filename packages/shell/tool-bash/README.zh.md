...
| `audit_root` | string | 启用 `auditFullAccessWrites` 时公开。Full Access 下可指向磁盘任意位置的现有绝对目录，必须包含本次前台调用可能修改的所有路径；默认使用 `workdir`。读取不受限制，根目录外写入会被拒绝。 |
