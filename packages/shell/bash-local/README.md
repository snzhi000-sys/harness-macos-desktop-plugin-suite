...
- **Foreground tree settlement** — after the direct foreground process closes, `run()` waits for the complete managed process tree for one `graceMs`. A surviving helper is terminated and the call rejects with `SHELL_PROCESS_TREE_SURVIVED`; it cannot be reported as a completed foreground command while continuing to modify files.
