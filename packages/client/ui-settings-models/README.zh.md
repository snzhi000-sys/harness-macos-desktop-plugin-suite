...
模型发现选择器可以按模型 id 或显示名称过滤。全选和取消全选只作用于当前过滤结果，并保留被过滤隐藏项目的选择状态。

本设置区拥有两个 root 作用域子槽位。`settings.models.provider-card` 按提供方设置命名空间分发，只提供目录事实（`provider`、`configured` 与 `keyConfigured`）；`settings.models.footer` 是页面底部的非 keyed 列表。Provider 认证插件因此可以增加路由专属登录控件，而无需接管模型设置页或 Better Sidebar。
