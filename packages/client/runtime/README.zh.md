...
`ctx.startupTasks` 是非关键浏览器启动 I/O 的协作队列。它在两个动画帧后开放，并为后台页面提供 200 ms 兜底；提交的操作按顺序串行执行，避免布局恢复和插件校准与首屏 Session 历史、Workspace/Explorer 读取争抢。调用方提供 `AbortSignal`，并继续负责自己的 generation 或 revision 检查；队列被销毁时，尚未开始的任务会直接完成而不执行，单个任务失败也不会阻塞后续任务。
