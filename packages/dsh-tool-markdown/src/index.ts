...
function normalizeMaxBytes(maxBytes: unknown): number {
...
function assertSize(input: unknown, maxBytes: number, label: string): asserts input is string {
...
function runAction(args: MarkdownArgs): string {
...
export function apply(ctx: Context): void {
  ctx.tools.register(defineTool({
    name: 'markdown',
    description:
      'Markdown utilities over text (zero-dependency, pure functions). ' +
    ...
    execute: args => Promise.resolve(runAction(args as MarkdownArgs)),
    timeoutMs: 2000,
  }))
}
