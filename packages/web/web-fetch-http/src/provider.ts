 ...
 *
 * Every request hop resolves and validates the complete public DNS answer set,
 * then pins the transport to those addresses to prevent DNS rebinding.
...
import type { HttpFetchResolver, HttpResponse, PinnedResponse } from './network.ts'
...
export class HttpFetchProvider implements WebFetchProvider {
    ...
    for (;;) {
      ...
      try {
        if (isRedirectStatus(response.status)) {
          ...
          // Re-validate the target against the same transport hygiene a direct request gets.
  ...
  private async requestOnce(url: URL, signal: AbortSignal): Promise<PinnedResponse> {
    try {
  ...
  private async readBody(response: HttpResponse, finalUrl: URL, signal: AbortSignal): Promise<WebFetchResult> {
   ...
   */
  private async readCapped(response: HttpResponse, signal: AbortSignal): Promise<{ bytes: Uint8Array; truncatedByBytes: boolean }> {
