...
function collectImageLengths(blocks: readonly ContentBlock[], lengths: number[]): void {
  for (const block of blocks) {
    if (block.type === 'image') lengths.push(base64Length(block.attachment.bytes))
    else if (block.type === 'tool-result') collectImageLengths(block.content, lengths)
  }
}

function replaceOldestImages(blocks: readonly ContentBlock[], remaining: { count: number }): ContentBlock[] {
 ...
 * their accumulated base64 payload fits the configured bound. Durable
 * messages and attachment references are never mutated.
 */
