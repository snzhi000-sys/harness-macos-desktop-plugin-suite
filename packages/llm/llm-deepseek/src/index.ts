...
export interface Config {
  ...
  /** Maximum accumulated inline image payload after Files API fallback. */
  maxRequestImageBytes?: number
  /** Explicit lifetime assigned to uploaded image files. */
  fileExpiresAfterSeconds?: number
  /** Remaining lifetime below which a cached file id is refreshed. */
...
export const Config: z<Config> = z.object({
  ...
  fileExpiresAfterSeconds: z.number().step(1).min(3_600).max(2_592_000).default(7 * 24 * 60 * 60),
  fileRefreshMarginSeconds: z.number().step(1).min(0).default(60 * 60),
...
export function resolveAdapterOptions(config: Config, environment?: LaunchEnvironmentSnapshot): ResolvedDeepSeekOptions {
  ...
  const fileExpiresAfterSeconds = config.fileExpiresAfterSeconds ?? 7 * 24 * 60 * 60
  const fileRefreshMarginSeconds = config.fileRefreshMarginSeconds ?? 60 * 60
  ...
  return {
    ...
    filePolicy: { expiresAfterSeconds: fileExpiresAfterSeconds, refreshMarginSeconds: fileRefreshMarginSeconds },
