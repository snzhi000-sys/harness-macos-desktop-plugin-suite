...
describe('DeepSeekLlmApiExtensionRegistry', () => {
  it('prepares detached frozen fields and accepts every provider exactly once', async () => {
    const ctx = await harness()
    const first = vi.fn()
    const second = vi.fn()
    const mutable = { value: 'original' }
    ctx.deepseekLlmApiExtensions.register('test_alpha', {
      prepare: () => ({ value: mutable, accept: first }),
    })
  ...
  it('settles every acceptance callback before reporting multiple failures', async () => {
    const ctx = await harness()
    const later = vi.fn()
    ctx.deepseekLlmApiExtensions.register('test_alpha', {
      prepare: () => ({ value: { value: 'x' }, accept: () => { throw new Error('alpha failed') } }),
    })
    ctx.deepseekLlmApiExtensions.register('test_beta', {
      prepare: () => ({ value: [2], accept: () => { later(); throw new Error('beta failed') } }),
    })
    const prepared = await ctx.deepseekLlmApiExtensions.prepare({ body: {}, signal: SIGNAL })
  ...
  it('reports a single acceptance failure verbatim and omits undefined contributions', async () => {
    const ctx = await harness()
  ...
  it('rejects invalid field names and preparation failures', async () => {
    const ctx = await harness()
  ...
  it('stops waiting for a provider that ignores request cancellation', async () => {
    ...
    const pending = ctx.deepseekLlmApiExtensions.prepare({ body: {}, signal: controller.signal })
