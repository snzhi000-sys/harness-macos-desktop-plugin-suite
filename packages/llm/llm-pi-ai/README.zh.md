...
对于已配置路由，模型发现会同时复用该路由的 Profile `headers` 与已保存凭据。表单中临时输入的 key 会覆盖已保存 Bearer token，但继续保留部署 header。发现请求在合并 Profile 后强制设置 JSON `Accept` 和 Harness 归因 header，因此 Profile 不能覆盖这些受控字段。浏览器侧发现不会接收 Profile header。
...
如果 listener 改写已组装 assistant 内容，loop 会在记录消息前丢弃回放状态，因为其提供方元数据不再描述该内容。无效版本、格式错误元数据，以及消息与回放状态之间的提供方／模型不匹配都会显式以 `LlmError('INVALID_REPLAY_STATE')` 失败。原生终止响应可能包含 Harness 流未定稿的不完整块；如果持久块仍是其有序子集，适配器会保留对应元数据，否则会把持久消息作为外部的、与提供方无关的内容回放，避免陈旧元数据阻断会话。
