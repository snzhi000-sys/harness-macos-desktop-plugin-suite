...
领域可选择 `per-record` 存储，并通过 `compatibleVersions` 列出当前 schema 仍可接受的旧记录版本；写入始终使用当前版本。权威领域保持 schema 校验失败即明确报错。可丢弃的派生数据领域可声明 `invalidRecords: 'backup-and-skip'`：打开时通过后端的 `backupRecord` 移走单个无效独立文档，仅记录记录位置、领域版本和错误分类，然后在不包含该记录的情况下继续。后端不支持记录备份时仍明确报错，全局值损坏也始终失败。
