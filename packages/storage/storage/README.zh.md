...
- 后端拥有一种介质，并公开其支持的数据形状**分面**。当前分面为 `kv`；其单元描述符可选择默认的 `single` 布局或可选的 `per-record` 布局，并可声明读取时接受的旧记录版本。拥有独立记录文档的后端可公开 `backupRecord`，用于恢复可丢弃的派生数据；`src/backend.ts` 负责定义确切约定。
