...
| `readHistoryTail(id, maxMessages, signal?): Promise<SessionHistoryTailInspection>` | 返回按追加来源消息边界切分的逻辑 transcript 尾页。默认实现执行 `inspect`；只有尾页来自此前已验证且 revision 匹配的日志时，优化后端才可省略完整 `inspection`。恢复、fork、模型重建与修复绝不使用此方法。 |
