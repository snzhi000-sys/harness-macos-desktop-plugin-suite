...
| `readHistoryTail(id, maxMessages, signal?): Promise<SessionHistoryTailInspection>` | Return a logical transcript tail cut at append-origin message boundaries. The default performs `inspect`; an optimized backend may omit the full `inspection` only when the returned tail was derived from a previously validated log revision. Resume, fork, model reconstruction, and repair never use this method. |
