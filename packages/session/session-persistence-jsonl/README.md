...
- **Validated history-tail cache.** A full cold inspection records a rebuildable 50-message sidecar beside the log. `readHistoryTail()` reuses it only when the main artifact's complete stat-derived revision and this build's supported-event set match; absence, corruption, append, repair, or format support changes fall back to full inspection. The cache never participates in resume, fork, model reconstruction, or repair.
