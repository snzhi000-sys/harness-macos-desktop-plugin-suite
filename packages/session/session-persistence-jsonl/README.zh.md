...
- **已验证历史尾页缓存。**一次完整冷 inspection 会在日志旁记录可重建的 50 条消息伴随文件。只有主产物完整的 stat 派生 revision 与当前构建支持的事件集合均匹配时，`readHistoryTail()` 才复用它；缓存缺失或损坏、日志追加或修复、格式支持变化都会回退完整 inspection。该缓存绝不参与恢复、fork、模型重建或修复。
