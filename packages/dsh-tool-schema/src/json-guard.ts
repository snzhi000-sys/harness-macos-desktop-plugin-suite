...
/** 栈帧：enter 为 true 时进入节点（统计/入 ancestor 集），false 时离开。 */
interface Frame {
  value: unknown
  path: string
  depth: number
...
/** 非字符串标量的 UTF-8 字节估算。 */
function scalarBytes(value: boolean | number | null): number {
 ...
 */
export function checkJsonValue(value: unknown, opts: GuardOptions): GuardResult {
  ...
  const fail = (code: string, message: string, path: string): GuardResult => ({
    ok: false,
    nodes,
    depth: maxDepthSeen,
    bytes,
  ...
  const stack: Frame[] = [{ value, path: '', depth: 0, enter: true }]
  while (stack.length > 0) {
    const frame = stack.pop()!
    const { value: current, path, depth } = frame

    if (!frame.enter) {
      if (typeof current === 'object' && current !== null) ancestors.delete(current)
      continue
    }
    ...
    if (!Array.isArray(obj) && !isJsonRecord(obj)) {
      return fail(
        `${label}-invalid`,
        `${label} contains a value that is not a plain JSON object or array (${obj.constructor?.name ?? 'unknown'})`,
        path,
      )
    }
    ancestors.add(obj)
...
export function isJsonCompatible(value: unknown): boolean {
  ...
  const visit = (v: unknown): boolean => {
    ...
    if (!Array.isArray(v) && !isJsonRecord(v)) return false
    seen.add(v)
    if (Array.isArray(v)) {
      for (const item of v) if (!visit(item)) return false
    } else {
      for (const key of Object.keys(v as Record<string, unknown>)) {
        if (!visit((v as Record<string, unknown>)[key])) return false
      }
    }
    seen.delete(v)
    return true
