...
import { MAX_EXPLAIN_NODES, MAX_SCHEMA_BYTES, MAX_DEPTH, MAX_SCHEMA_NODES } from './limits.ts'
import { checkJsonValue, escapeToken, isJsonRecord } from './json-guard.ts'
import { checkSchema } from './schema-check.ts'
import type { ExplainNode, ExplainResult, SchemaIssue } from './types.ts'
...
function summarize(node: Record<string, unknown>): string {
  ...
  if (Object.hasOwn(node, 'type')) {
    const t = node.type
    if (Array.isArray(t)) parts.push((t as string[]).join(' | '))
    else if (typeof t === 'string') parts.push(t)
  }
  if (Object.hasOwn(node, 'enum') && Array.isArray(node.enum)) {
    parts.push(`enum: ${(node.enum as unknown[]).map((v) => JSON.stringify(v)).join(', ')}`)
  }
  if (Object.hasOwn(node, 'const')) {
    parts.push(`const: ${JSON.stringify(node.const)}`)
  }
  if (Object.hasOwn(node, 'required') && Array.isArray(node.required) && (node.required as unknown[]).length > 0) {
    parts.push(`requires: ${(node.required as string[]).join(', ')}`)
  }
  ...
  if (node.uniqueItems === true) parts.push('uniqueItems')
  ...
  if (Object.hasOwn(node, 'items')) {
    ...
    if (typeof items === 'boolean') parts.push(`items: ${items ? 'any' : 'none'}`)
  ...
  if (Object.hasOwn(node, 'additionalProperties')) {
    ...
    if (typeof ap === 'boolean') parts.push(`additionalProperties: ${ap ? 'allowed' : 'forbidden'}`)
    else parts.push('additionalProperties: schema')
  }
  if (Object.hasOwn(node, 'allOf') && Array.isArray(node.allOf)) {
    parts.push(`allOf: ${(node.allOf as unknown[]).length} branches`)
  }
  if (Object.hasOwn(node, 'anyOf') && Array.isArray(node.anyOf)) {
    parts.push(`anyOf: ${(node.anyOf as unknown[]).length} branches`)
  }
  if (Object.hasOwn(node, 'oneOf') && Array.isArray(node.oneOf)) {
    parts.push(`oneOf: ${(node.oneOf as unknown[]).length} branches`)
  }
  if (Object.hasOwn(node, 'not')) {
    const not = node.not
    if (typeof not === 'boolean') parts.push(`not: ${not ? 'true' : 'false'}`)
...
interface ExplainWalkState {
  truncated: boolean
}

function walkExplain(schema: unknown, sp: string, nodes: ExplainNode[], state: ExplainWalkState): void {
  ...
  for (const comb of ['allOf', 'anyOf', 'oneOf'] as const) {
    if (Array.isArray(schema[comb])) {
      ;(schema[comb] as unknown[]).forEach((branch, i) => {
...
/** explain action。 */
export function explainSchema(schema: unknown, _opts: ExplainOptions): ExplainResult {
  ...
  const state: ExplainWalkState = { truncated: false }
  if (schemaGuard.ok) {
