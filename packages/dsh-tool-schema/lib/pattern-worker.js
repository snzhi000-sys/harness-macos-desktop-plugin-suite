 ...
 */
export function runPatternChecksSync(checks, budgetMs) {
    const matched = {};
 ...
 */
export function runPatternChecksInWorker(checks, budgetMs = PATTERN_BUDGET_MS) {
    return new Promise((resolve) => {
        const workerUrl = new URL(import.meta.url.endsWith('.ts') ? './pattern-worker.ts' : './pattern-worker.js', import.meta.url);
        ...
        worker.postMessage({ id: 1, checks, budgetMs });
    });
}
// ── worker 入口 ──
if (parentPort) {
    parentPort.on('message', (req) => {
        ...
        try {
            ...
            if (result.timedOut)
                error = `pattern budget exhausted (${req.budgetMs}ms)`;
        }
        catch (e) {
