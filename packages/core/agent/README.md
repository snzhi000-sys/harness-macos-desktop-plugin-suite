...
`AgentOptions` supplies the initial provider/model route, optional adapter-owned `reasoningEffort`, and optional positive `maxTokens` output cap. The concrete loop uses an explicit creation-time reasoning effort for the first request, resolves exact-model adapter defaults for omitted values, records the effective values in the request header, and applies them to each conversation-model request.
