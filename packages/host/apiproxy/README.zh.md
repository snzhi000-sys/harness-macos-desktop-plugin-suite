...
`session.history` 会读取已附加 Session 的内存状态，或通过持久化读取已验证的冷历史尾页，而不会恢复或发布 agent，然后按追加来源的消息边界分页。只有 projection cache 的切点与尾页最终 seq 相同时，网关才采用后端尾页；否则继续执行既有的完整 inspection。`maxMessages` 统计以追加方式进入 surface 的 `user/message` 和 `assistant/message` 事件，因此仅供模型使用的替换副本不占用配额。每一页仍是一段连续的原始事件区间，从而让压缩（compaction）的仅日志 `compaction/summary` 记录与引用它的替换留在同一页。
