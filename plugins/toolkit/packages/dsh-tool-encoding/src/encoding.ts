...
const ALGORITHMS: Record<string, true> = { md5: true, sha1: true, sha256: true, sha512: true }
...
function validateUnicode(input: string): void {
  if (!input.isWellFormed()) throw new Error('encoding: invalid Unicode (lone surrogate)')
}

/** 输入字节上限（UTF-8 字节数，非 UTF-16 code unit 数）。 */
function assertInputBytes(input: string): void {
...
/** 输出字节上限（编码膨胀防护）。 */
function assertOutputBytes(output: string): void {
...
function assertBase64(input: string): void {
  if (!BASE64_RE.test(input)) throw new Error('encoding: invalid base64 input')
  // 长度非 4 倍数一律拒绝（含模 4 余 1/2/3）：Node 的宽容解码会静默截断，必须自己拦
  if (input.length % 4 !== 0) throw new Error('encoding: invalid base64 input')
  ...
  if (dataLen % 4 === 2) {
    ...
    if (second === -1 || (second & 0b1111) !== 0) throw new Error('encoding: non-canonical base64 input')
  } else if (dataLen % 4 === 3) {
    const third = BASE64_ALPHABET.indexOf(input.charAt(dataLen - 1))
    if (third === -1 || (third & 0b11) !== 0) throw new Error('encoding: non-canonical base64 input')
  }
}

/** hex 输入校验（偶数长度 + 全 hex 字符集）。 */
function assertHex(input: string): void {
  if (input.length % 2 !== 0 || !HEX_RE.test(input)) throw new Error('encoding: invalid hex input')
}
...
function b64UrlDecode(input: string): string {
  ...
  if (firstPadding !== -1) {
    ...
    if (!/^={1,2}$/.test(padding)) throw new Error('encoding: invalid base64 input')
    padded = data + padding
    if (padded.length % 4 !== 0) throw new Error('encoding: invalid base64 input')
  } else {
    if (normalized.length % 4 === 1) throw new Error('encoding: invalid base64 input')
...
export function executeAction(action: unknown, params: unknown): string {
  ...
  switch (action) {
    ...
    case 'hash': {
      if (typeof actionParams.algorithm !== 'string') throw new Error('encoding: missing required parameter "algorithm"')
      if (typeof actionParams.input !== 'string') throw new Error('encoding: missing required parameter "input"')
    ...
    case 'hex_encode': {
      ...
      if (worstCase > MAX_OUTPUT_BYTES) throw new Error('encoding: output would exceed limit')
