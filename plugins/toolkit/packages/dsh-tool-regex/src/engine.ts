...
export function executeAction(args: RegexActionArgs): string {
  ...
  switch (args.action) {
    case 'test': {
      ...
      value = testMatch(re, args.input as string)
      break
    }
    case 'find': {
      assertInputSize(args.input)
      ...
      value = findAll(re, args.input as string, limit)
      break
    }
    case 'replace': {
      assertInputSize(args.input)
      const re = compilePattern(args.pattern, flags, { forceGlobal: true })
      value = replaceAll(re, args.input as string, args.replacement)
      break
    }
    case 'explain': {
