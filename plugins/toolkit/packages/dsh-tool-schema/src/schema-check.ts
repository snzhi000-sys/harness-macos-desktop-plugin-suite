...
import { MAX_PATTERN_BYTES, MAX_PATTERNS, MAX_REF_CHAIN } from './limits.ts'
import { escapeToken } from './json-guard.ts'
import { parseRef, resolveRef } from './ref.ts'
...
interface CheckCtx {
  ...
  validPatterns: Set<string>
  patternCount: number
  /** schema 根（`$ref` 解析基准；所有本地引用均锚定在根）。 */
  root: unknown
}

function issue(ctx: CheckCtx, schemaPath: string, keyword: string, code: string, message: string): void {
  ctx.issues.push({ schemaPath, keyword, code, message })
}

/** 记录节点路径（首个遇到者胜）。 */
function notePath(ctx: CheckCtx, node: object, schemaPath: string): void {
  if (!ctx.nodePaths.has(node)) ctx.nodePaths.set(node, schemaPath)
}

/** 关键字类型校验 + 子 schema 递归。返回 false 表示已跳过递归（结构非法）。 */
function checkKeyword(ctx: CheckCtx, node: Record<string, unknown>, key: string, sp: string): void {
  ...
  switch (key) {
    ...
    case 'pattern': {
      ...
      try {
        ...
        issue(ctx, kwPath, key, 'pattern-invalid', `invalid regular expression: ${String((error as Error).message)}`)
      }
      break
    }
    case 'minimum':
 ...
 */
function checkRefStatic(ctx: CheckCtx, ref: string, kwPath: string): void {
  ...
  let target: unknown = resolved.value
  const chain: unknown[] = [target]
  while (typeof target === 'object' && target !== null && !Array.isArray(target)) {
    const keys = Object.keys(target)
    if (!(keys.length === 1 && keys[0] === '$ref')) break
    const nextRef = (target as Record<string, unknown>).$ref
    if (typeof nextRef !== 'string') break
...
function walk(ctx: CheckCtx, node: unknown, sp: string): void {
 ...
 */
export function checkSchema(schema: unknown): CheckSchemaResult {
  const ctx: CheckCtx = {
    issues: [],
