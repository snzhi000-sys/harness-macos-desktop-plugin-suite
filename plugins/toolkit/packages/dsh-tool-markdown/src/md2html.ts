...
function scanUrlEnd(input: string, openIdx: number): number {
  let depth = 0
  let j = openIdx
 ...
 */
function renderInline(input: string, options: Md2HtmlOptions, allowBold = true): string {
  let out = ''
  let i = 0
    ...
    if (c === '!' && input[i + 1] === '[') {
      ...
      if (close !== -1) {
        ...
        if (closeEnd !== -1) {
          ...
          i = closeEnd + 1
          if (url !== '' && safeScheme(url)) out += `<img src="${escapeHtml(url)}" alt="${escapeHtml(alt)}">`
          else out += escapeHtml(alt)
          continue
        }
      }
    }
...
function renderTable(lines: string[], options: Md2HtmlOptions): string {
  const rows: string[][] = lines.map(splitPipe)
  // 找分隔行
  let sepIndex = -1
  ...
  const tr = (cells: string[], tag: 'th' | 'td'): string =>
    `<tr>${pad(cells).map(c => `<${tag}>${renderInline(c, options)}</${tag}>`).join('')}</tr>`
...
/** Markdown → 白名单 HTML。 */
export function md2html(markdown: string, options: Md2HtmlOptions = {}): string {
