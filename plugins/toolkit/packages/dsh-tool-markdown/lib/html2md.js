...
function escapeLineStart(text) {
    if (/^[#>+\-]/.test(text))
        return `\\${text}`;
    if (/^\d+\./.test(text))
        return text.replace(/^(\d+)\./, '$1\\.'); // 转义点号破坏有序列表
    return text;
}
...
function renderInline(node, options) {
    if (node.type === 'text')
        return escapeInlineText(collapse(node.text ?? ''));
    const tag = node.tag ?? '';
    ...
    switch (tag) {
        ...
        case 'a': {
            ...
            if (href === '' || !safeScheme(href))
                return children; // javascript:/data: 降级纯文本
        ...
        case 'img': {
            ...
            if (src === '' || !safeScheme(src))
                return alt; // 危险 scheme：只留 alt
            const title = node.attrs['title'];
    ...
    switch (tag) {
        ...
        default:
            ...
            return node.children.map(c => {
                if (c.type === 'text')
                    return escapeLineStart(collapse(c.text ?? ''));
                return renderBlock(c, options);
            }).filter(Boolean).join('\n\n');
    }
}
function renderList(node, depth, options) {
...
export function tableToGfm(node, options = {}) {
    ...
    const fmt = (cells) => `| ${cells.map(c => escapePipe(collapse(c).trim())).join(' | ')} |`;
