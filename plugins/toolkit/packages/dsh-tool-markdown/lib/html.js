...
function parseTagName(input, start) {
 ...
 */
export function parseHtml(input, options = {}) {
    ...
    while (i < len) {
        ...
        while (j < len) {
            ...
            j = k;
            while (j < len && WHITESPACE.has(input[j]))
                j++;
            if (input[j] === '=') {
                j++;
                while (j < len && WHITESPACE.has(input[j]))
                ...
                if (q === '"' || q === "'") {
                    ...
                    if (close === -1) {
                        attrs[name] = decodeEntities(input.slice(j + 1));
                        j = len;
                    }
                    else {
                        attrs[name] = decodeEntities(input.slice(j + 1, close));
                        j = close + 1;
                    }
                }
                else {
        ...
        const node = { type: 'element', tag: tag.name, attrs, children: [] };
