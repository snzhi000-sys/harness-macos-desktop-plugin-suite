...
export type JsonQueryErrorCode =
  | 'MISSING_PROPERTY'
  | 'TYPE_MISMATCH'
  | 'INDEX_OUT_OF_BOUNDS'
  | 'INVALID_QUERY'
...
function parseQuery(input: string): QueryStep[] {
  if (input.length > MAX_LENGTH) throw new JsonQueryError('INVALID_QUERY', `Query too long (${input.length} > ${MAX_LENGTH})`)
  if (input === '') return []
  ...
  const err = (msg: string): never => { throw new JsonQueryError('INVALID_QUERY', `json: ${msg} at position ${pos}`) }
  ...
  while (pos < input.length) {
    ...
    if (peek() === '[') {
      ...
      if (peek() === "'" || peek() === '"') {
        ...
        while (pos < input.length) {
          ...
          if (c === '\\') {
            ...
            if (next === '\\' || next === "'" || next === '"') { key += next; pos += 2; continue }
      ...
      if (/[0-9]/.test(peek() ?? '')) {
        ...
        if (!Number.isSafeInteger(idx)) err('array index is too large')
...
function executeQuery(steps: QueryStep[], value: unknown): unknown {
  ...
  for (let i = 0; i < steps.length; i++) {
    const step = steps[i] as QueryStep
    ...
    if (step.kind === 'index') {
      ...
      if (step.index >= (current as unknown[]).length) {
        throw new JsonQueryError('INDEX_OUT_OF_BOUNDS', `json: index ${step.index} out of bounds (length ${(current as unknown[]).length})`)
      }
    ...
    if (remaining.length === 0) {
      ...
      for (const item of arr) {
        if (item === null || item === undefined || typeof item !== 'object') continue // 非对象元素跳过
        try {
          projected.push(executeQuery(remaining, item))
        } catch (error) {
          if (error instanceof JsonQueryError && error.code === 'MISSING_PROPERTY') continue
          throw error
        }
      }
      current = projected
...
/** 字节累计状态（RECHECK2-JSON-01：对象路径与字符串路径统一大小上限）。 */
interface SizeState { bytes: number }

/**
 * 累计 UTF-8 字节并即时检查上限（AUDIT-JSON-01：所有标量和结构分支统一检查，
 * 不再只在 string/key 分支触发——纯 number/boolean/null 数组也能被拦截）。
 */
function addBytes(state: SizeState, amount: number): void {
  state.bytes += amount
...
function assertJsonCompatible(
  ...
  if (depth > 100) throw new JsonQueryError('INVALID_QUERY', 'json: input nesting too deep')
  if (value === null) { addBytes(state, 4); return }
  if (typeof value === 'string') {
    addBytes(state, Buffer.byteLength(value, 'utf8') + 2) // 引号开销
    return
  }
  if (typeof value === 'boolean') { addBytes(state, 5); return }
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) throw new JsonQueryError('INVALID_QUERY', 'json: non-finite number in input')
  ...
  try {
    ...
    if (ownKeys.length > 0) addBytes(state, 2) // { }
    ownKeys.forEach((key, index) => {
      if (index > 0) addBytes(state, 1) // 逗号
      const descriptor = Object.getOwnPropertyDescriptor(value, key)
      if (descriptor === undefined) throw new JsonQueryError('INVALID_QUERY', 'json: input must be JSON-compatible')
...
export function normalizeInput(input: unknown): unknown {
  if (typeof input === 'string') {
    ...
    try {
      ...
      if (error instanceof JsonQueryError) throw error // 深度/类型错误如实透传
...
export function query(jsonInput: unknown, queryStr: unknown): unknown {
