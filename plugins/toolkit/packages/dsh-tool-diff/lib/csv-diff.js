...
export function parseCsv(text, delimiter = ',') {
    ...
    if (delimiter.length !== 1)
        throw new Error('diff: delimiter must be a single character');
...
function cellsEqual(a, b, ignoreCase, ignoreWhitespace) {
    if (a.length !== b.length)
        return false;
    for (let i = 0; i < a.length; i++) {
        if (normalizeCell(a[i], ignoreCase, ignoreWhitespace) !== normalizeCell(b[i], ignoreCase, ignoreWhitespace))
            return false;
    }
    return true;
}
function resolveKeyIndex(header, key) {
    const num = Number(key);
    if (!Number.isNaN(num) && /^\d+$/.test(key)) {
        const idx = num - 1;
        if (idx < 0 || idx >= (header?.length ?? 0))
            throw new Error(`diff: key column index ${key} out of range (${header?.length ?? 0} columns)`);
        return idx;
    }
    if (header === null)
        throw new Error(`diff: named key "${key}" requires a header row`);
    const idx = header.indexOf(key);
    if (idx === -1)
        throw new Error(`diff: key column "${key}" not found in header ${JSON.stringify(header)}`);
    return idx;
}
/** 把 cells 转成以表头为键的对象；缺失列 → null；多余列 → columnN。 */
function toRowObject(cells, header) {
    const obj = {};
...
export function csvDiff(beforeText, afterText, options = {}) {
    ...
    if (after.header === null || options.key === undefined) {
        ...
        for (let i = 0; i < max; i++) {
            ...
            const id = String(i + 1);
            if (!rb)
                report.addedRows.push({ row: id, cells: ra.cells });
            else if (!ra)
                report.removedRows.push({ row: id, cells: rb.cells });
    ...
    if (beforeHeader) {
        for (const c of header)
            if (!beforeHeader.includes(c))
                report.addedColumns.push(c);
        for (const c of beforeHeader)
            if (!header.includes(c))
                report.removedColumns.push(c);
    }
    else {
    ...
    const dupSet = new Set([...beforeIdx.dupKeys, ...afterIdx.dupKeys]);
    ...
    for (const rb of before.rows) {
        const k = dupKeyOf(rb);
        if (dupSet.has(k))
            continue; // 重复 key 的行结果不确定，跳过
        const ra = afterIdx.map.get(k);
        if (!ra) {
            report.removedRows.push(toRowObject(rb.cells, header));
            continue;
        }
        if (cellsEqual(rb.cells, ra.cells, ignoreCase, ignoreWhitespace))
            continue;
        const changes = [];
        ...
        for (let c = 0; c < width; c++) {
            ...
            if (bv === undefined)
                changes.push({ column: colName, before: null, after: av });
            else if (av === undefined)
                changes.push({ column: colName, before: bv, after: null });
...
function positionalCellChanges(before, after, ignoreCase, ignoreWhitespace) {
    const changes = [];
    ...
    for (let c = 0; c < width; c++) {
        ...
        const av = after[c];
        if (bv === undefined)
            changes.push({ column: `column${c + 1}`, before: null, after: av });
        else if (av === undefined)
            changes.push({ column: `column${c + 1}`, before: bv, after: null });
        else if (normalizeCell(bv, ignoreCase, ignoreWhitespace) !== normalizeCell(av, ignoreCase, ignoreWhitespace)) {
