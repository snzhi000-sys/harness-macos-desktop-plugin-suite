...
describe('time: plugin registration contract (AUDIT-CROSS-02)', () => {
  it('exports the cordis plugin contract', () => {
    expect(name).toBe('@deepseek-ai/dsh-tool-time')
    expect(inject).toContain('tools')
    expect(typeof apply).toBe('function')
  })

  it('registers the time tool with schema + render', () => {
    ...
    expect(def.parameters.action.enum).toEqual(['now', 'convert', 'add', 'diff'])
