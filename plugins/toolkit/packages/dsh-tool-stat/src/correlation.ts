...
function pearsonCore(method: CorrelationMethod, x: number[], y: number[]): CorrelationResult {
  const n = x.length
  let meanX = 0
  let meanY = 0
  let m2x = 0
...
export function correlationValues(x: number[], y: number[], method: CorrelationMethod): CorrelationResult {
  if (method === 'spearman') {
