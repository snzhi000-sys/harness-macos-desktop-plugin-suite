...
import {
  ...
  IconSparkle16, IconTrashOutline16, IconTriangleRightFill14, Menu, StateDot,
...
function SessionStatusDots({ statuses, size = 10 }: {
  statuses: readonly [SessionStatus, ...SessionStatus[]]
  size?: number | undefined
}) {
  return (
    <>
      <StateDot state={statuses[0].state} size={size} />
      {statuses.map(status => (
...
export function SessionNodeItem({
  ...
  lineageExpanded, onToggleLineage, t,
}: {
  node: SessionNode
  currentId: string | undefined
  now: number
  ...
  onRename: (id: SessionNode['id'], currentTitle: string, branch: boolean) => void
  ...
  /** Root rows own the only visual expand state; branch rows are always leaves. */
  lineageExpanded?: boolean | undefined
  onToggleLineage?: (() => void) | undefined
  t: RowTranslate
}) {
  const row = node
  ...
  const statuses = sessionStatuses(node, t)
  const primaryStatus = statuses[0]
  const isLineageRoot = node.lineage?.role === 'root'
  const isLineageBranch = node.lineage?.role === 'branch'
  const isRecentBranch = isLineageBranch && node.lineage?.recent === true
  // Idle stays in the hover card instead of adding noise to every row. Active,
  // attention-required, and newly completed states remain visible in-place;
  // lineage roots badge the arrow while branches use the normal status slot.
  ...
  const ownRow = (
    <div
      className={clsx(
        ...
        isLineageRoot && css.lineageRoot,
        isLineageBranch && css.lineageBranch,
    ...
    >
      ...
      {isLineageRoot && (
        <button
          type="button"
          className={css.lineageToggle}
          aria-label={lineageExpanded === true ? '收起会话分支' : '展开会话分支'}
          aria-expanded={lineageExpanded === true}
          onClick={(event) => {
            event.stopPropagation()
            onToggleLineage?.()
          }}
        >
          <IconTriangleRightFill14 className={clsx(css.arrow, lineageExpanded === true && css.arrowOpen)} />
          {showStatus && (
            <span className={css.lineageStatusBadge}>
              <SessionStatusDots statuses={statuses} size={6} />
            </span>
          )}
        </button>
      )}
      {!isLineageRoot && (!flat || showStatus) && (
        <span className={css.slot}>
      ...
      {!row.blank && (isRecentBranch
        ? (
            <span className={clsx(css.time, css.recentBranch)} title="最近对话" aria-label="最近对话">
      ...
      {!row.blank && (
        <span className={css.rowActions}>
          <Menu
            ...
            onSelect={(id) => {
              setMenuOpen(false)
              if (id === 'rename') onRename(node.id, row.title, isLineageBranch)
              if (id === 'fork') onFork(node.id)
              if (id === 'archive') {
                for (const sessionId of node.lineage?.sessionIds ?? [node.id]) onArchive(sessionId)
              }
            }}
            portal
            closeOnPointerLeave
