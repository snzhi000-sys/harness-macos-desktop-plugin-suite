...
describe('WorkspaceBrowser', () => {
  ...
  it('promotes a whole parent lineage when any child becomes the most recently active session', async () => {
    const root = summary('root', 1)
    const child = summary('child', 2, { parentId: root.id })
    const other = summary('other', 8)
    const workspaceView = workspaceState([workspace('alpha', ['root', 'child', 'other'])])
    const b = mount({
      useSessions: hook(sessionState([root, child, other])),
      useWorkspaces: hook(workspaceView),
    })
    fireEvent.click(screen.getByText('alpha'))
    fireEvent.click(screen.getByRole('button', { name: '视图选项' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '最近更新' }))
    await waitFor(() => {
      expect(b.store.getSnapshot().sessionOrderByAccount.alpha).toEqual(['other', 'root', 'child'])
    })
    expect(screen.getAllByRole('treeitem').slice(1)[0]?.textContent).toContain('other')

    const activeChild = { ...child, updatedAt: 10 }
    rerender(b, { useSessions: hook(sessionState([root, activeChild, other])) })
    await waitFor(() => {
      expect(b.store.getSnapshot().sessionOrderByAccount.alpha).toEqual(['root', 'child', 'other'])
      expect(screen.getAllByRole('treeitem').slice(1)[0]?.textContent).toContain('root')
    })
    expect(b.store.getSnapshot().sessionUpdatedAtByAccount.alpha).toEqual({ root: 10, other: 8 })

    fireEvent.click(screen.getByRole('button', { name: '视图选项' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '单列表' }))
    expect(screen.getAllByRole('treeitem')[0]?.textContent).toContain('root')
  })

  it('records child activity without moving its parent while Manual ordering is active', async () => {
    const root = summary('root', 1)
    const child = summary('child', 2, { parentId: root.id })
    const other = summary('other', 8)
    const b = mount({
      useSessions: hook(sessionState([root, child, other])),
      useWorkspaces: hook(workspaceState([workspace('alpha', ['other', 'root', 'child'])])),
    })
    fireEvent.click(screen.getByText('alpha'))
    await waitFor(() => {
      expect(b.store.getSnapshot().sessionOrderByAccount.alpha).toEqual(['other', 'root', 'child'])
    ...
    rerender(b, { useSessions: hook(sessionState([root, { ...child, updatedAt: 10 }, other])) })
    await waitFor(() => {
      expect(b.store.getSnapshot().sessionUpdatedAtByAccount.alpha).toEqual({ other: 8, root: 10 })
    })
    expect(b.store.getSnapshot().sessionOrderByAccount.alpha).toEqual(['other', 'root', 'child'])
    expect(screen.getAllByRole('treeitem').slice(1)[0]?.textContent).toContain('other')
  })
  ...
  it('projects a fork child beneath its root without changing branch drag behavior', () => {
    ...
    expect(screen.getByRole('button', { name: '收起会话分支' })).toBeTruthy()
    expect(screen.getByText('对话分支 A').closest('[role="treeitem"]')?.getAttribute('draggable')).toBe('false')
  })
