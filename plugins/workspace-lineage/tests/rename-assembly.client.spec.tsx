...
describe('session rename through the assembled browser', () => {
  ...
  it('renames a child branch and persists its custom projection title', async () => {
    const runtime = await createRuntime()
    const rootId = 'root' as SessionId
    const branchId = 'branch' as SessionId
    const rootRename = vi.fn<ISession['rename']>()
    const branchRename = vi.fn<ISession['rename']>(async title => ({
      ok: true, value: { title, seq: 3 },
    }))
    const branchTitleWrites: unknown[] = []
    vi.stubGlobal('fetch', vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      const path = String(input)
      if (path === '/workspace-lineage/branch-titles' && init?.method === 'POST') {
        const body = JSON.parse(String(init.body)) as { sessionId: string; title: string }
        branchTitleWrites.push(body)
        return new Response(JSON.stringify({ titles: { [body.sessionId]: body.title } }), {
          status: 200, headers: { 'content-type': 'application/json' },
        })
      }
      if (path === '/workspace-lineage/branch-titles') {
        return new Response(JSON.stringify({ titles: {} }), { status: 200 })
      }
      if (path === '/message-edit?lineages=1') {
        return new Response(JSON.stringify({ versions: [] }), { status: 200 })
      }
      if (path === '/sidebar/api/session-titles.get') {
        return new Response(JSON.stringify({ titles: {} }), { status: 200 })
      }
    ...
    await runtime.sessions.add({
      id: rootId,
      summary: { title: '父级会话', displayTitle: '父级会话', cwd: '/w/alpha' },
      session: { rename: rootRename },
    })
    await runtime.sessions.add({
      id: branchId,
      summary: {
        title: '原始内部标题', displayTitle: '原始内部标题', cwd: '/w/alpha', parentId: rootId,
      },
      session: { rename: branchRename },
    })
    await runtime.workspaces.update((draft) => {
      draft.items = [{
        workspaceId: 'w1' as WorkspaceId, title: 'alpha', path: '/w/alpha',
        sessionIds: [rootId, branchId], createdAt: '2026-01-01T00:00:00.000Z', updatedAt: '2026-01-01T00:00:00.000Z',
      }] as never
    })
    await runtime.root.declare(
      { 'sidebar.workspaces': { kind: 'single', scope: 'root' } } as never,
    ...
    const rootRow = (await view.findByText('父级会话')).closest('[role="treeitem"]')!
    const toggle = within(rootRow as HTMLElement).queryByRole('button', { name: '展开会话分支' })
    if (toggle !== null) fireEvent.click(toggle)
    const branchRow = (await view.findByText('对话分支 A')).closest('[role="treeitem"]')!
    fireEvent.click(within(branchRow as HTMLElement).getByLabelText('会话“对话分支 A”的操作'))
    fireEvent.click(view.getByRole('menuitem', { name: '重命名', hidden: true }))
    fireEvent.change(await view.findByLabelText('会话名称'), { target: { value: '接口改造方案' } })
    fireEvent.click(view.getByRole('button', { name: '重命名' }))

    await waitFor(() => { expect(branchRename).toHaveBeenCalledWith('接口改造方案') })
    await waitFor(() => {
      expect(branchTitleWrites).toEqual([{ sessionId: branchId, title: '接口改造方案' }])
    })
    await view.findByText('接口改造方案')
    expect(rootRename).not.toHaveBeenCalled()
    expect(view.queryByText('对话分支 A')).toBeNull()
    await runtime.dispose()
  })
