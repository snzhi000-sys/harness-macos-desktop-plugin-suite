...
import {
  conversationOrderUnits, deriveFlat, deriveGroups, deriveSearchResults, workspaceLabel, relativeTime,
...
const forked = (id: string, parentId: string, updatedAt: number): SessionSummary => ({
...
describe('deriveGroups', () => {
  ...
  it('hides subagent-origin sessions without absorbing a fork whose direct parent is a subagent', () => {
  ...
  it('projects ordinary forks into the original conversation and leaves invalid ancestry top-level', () => {
    const parent = summary('parent', 1)
    const oldChild = forked('old-child', 'parent', 10)
    const newChild = forked('new-child', 'parent', 20)
    const nestedChild = forked('nested-child', 'old-child', 15)
    ...
    const groups = deriveGroups(
      list(parent, oldChild, nestedChild, newChild, self, orphan, cycleA, cycleB),
      [],
      noArchive,
    ...
    const lineage = groups[0]!.sessions.at(-1)!
    expect(lineage.lineage?.sessionIds).toEqual([
      parent.id, oldChild.id, nestedChild.id, newChild.id,
    ])
    expect(lineage.branches.map(node => [node.id, node.title])).toEqual([
      [newChild.id, '对话分支 B'],
      [nestedChild.id, '对话分支 A-1'],
      [oldChild.id, '对话分支 A'],
    ])
  ...
  it('overrides only a branch display title while preserving its stable lineage identity', () => {
    const root = summary('root', 1)
    const child = forked('child', 'root', 2)
    const sessions = list(root, child)
    const grouped = deriveGroups(
      sessions,
      [workspace('project', ['root', 'child'])],
      noArchive,
      { expandedGroups: ['project'], branchTitles: { child: '接口改造方案' } },
    )
    const groupedBranch = grouped[0]!.sessions[0]!.branches[0]!
    expect(groupedBranch.title).toBe('接口改造方案')
    expect(groupedBranch.lineage).toMatchObject({ role: 'branch', branchName: 'A' })

    const flatBranch = deriveFlat(sessions, noArchive, { child: '接口改造方案' })[0]!.branches[0]!
    expect(flatBranch.title).toBe('接口改造方案')
    expect(flatBranch.lineage?.branchName).toBe('A')
  })

  it('projects every nested message-edit version as a direct child of the original session', () => {
    const root = { ...summary('root', 1), displayTitle: '原始会话（编辑前版本）' }
    const branchA = edited('branch-a', 'root', 2, 3)
    const branchA1 = edited('branch-a-1', 'branch-a', 3, 4)
    const branchA2 = edited('branch-a-2', 'branch-a-1', 4, 5)
    const branchB = edited('branch-b', 'root', 5, 2)
    const unrelatedSameTitle = { ...summary('unrelated', 6), displayTitle: '原始会话' }
    const sessions = { ...list(root, branchA, branchA1, branchA2, branchB, unrelatedSameTitle), current: branchB.id }
    const groups = deriveGroups(
      sessions,
      [workspace('project', ['root', 'branch-a', 'branch-a-1', 'branch-a-2', 'branch-b', 'unrelated'])],
      noArchive,
      view(['project']),
    )

    expect(groups[0]!.sessions.map(node => node.id)).toEqual([root.id, unrelatedSameTitle.id])
    const lineage = groups[0]!.sessions[0]!
    expect(lineage.title).toBe('原始会话')
    expect(lineage.lineage?.sessionIds).toEqual([
      root.id, branchA.id, branchA1.id, branchA2.id, branchB.id,
    ])
    expect(lineage.branches.map(node => [node.id, node.title])).toEqual([
      [branchB.id, '编辑分支 B'],
      [branchA2.id, '编辑分支 A-2'],
      [branchA1.id, '编辑分支 A-1'],
      [branchA.id, '编辑分支 A'],
    ])
    expect(lineage.branches.filter(node => node.lineage?.recent).map(node => node.id))
      .toEqual([branchB.id])
  })

  it('flattens mixed edit and ordinary-fork ancestry into exactly two visual levels', () => {
    const root = summary('root', 1)
    const editA = edited('edit-a', 'root', 2, 3)
    const forkA1 = forked('fork-a-1', 'edit-a', 3)
    const forkA2 = forked('fork-a-2', 'fork-a-1', 4)
    const forkB = forked('fork-b', 'root', 5)
    const sessions = { ...list(root, editA, forkA1, forkA2, forkB), current: forkB.id }
    const groups = deriveGroups(
      sessions,
      [workspace('project', ['root', 'edit-a', 'fork-a-1', 'fork-a-2', 'fork-b'])],
      noArchive,
      view(['project']),
    )

    expect(groups[0]!.sessions.map(node => node.id)).toEqual([root.id])
    expect(groups[0]!.sessions[0]!.branches.map(node => [node.id, node.title])).toEqual([
      [forkB.id, '对话分支 B'],
      [forkA2.id, '对话分支 A-2'],
      [forkA1.id, '对话分支 A-1'],
      [editA.id, '编辑分支 A'],
    ])
    expect(groups[0]!.sessions[0]!.branches.filter(node => node.lineage?.recent).map(node => node.id))
      .toEqual([forkB.id])
  })

  it('marks the only child as recent when a lineage has one child', () => {
    const root = summary('root', 1)
    const only = forked('only', 'root', 2)
    const groups = deriveGroups(
      list(root, only),
      [workspace('project', ['root', 'only'])],
      noArchive,
      view(['project']),
    )
    expect(groups[0]!.sessions[0]!.branches[0]!.lineage?.recent).toBe(true)
  })

  it('sorts branches by latest activity without changing their creation-based identities', () => {
    const root = summary('root', 1)
    const branchA = { ...edited('branch-a', 'root', 2, 2), updatedAt: 20 }
    const branchA1 = { ...edited('branch-a-1', 'branch-a', 3, 3), updatedAt: 30 }
    const branchB = { ...edited('branch-b', 'root', 4, 4), updatedAt: 40 }
    const groups = deriveGroups(
      list(root, branchA, branchA1, branchB),
      [workspace('project', ['root', 'branch-a', 'branch-a-1', 'branch-b'])],
      noArchive,
      view(['project']),
    )

    expect(groups[0]!.sessions[0]!.branches.map(branch => [branch.id, branch.title, branch.lineage?.branchName]))
      .toEqual([
        [branchB.id, '编辑分支 B', 'B'],
        [branchA1.id, '编辑分支 A-1', 'A-1'],
        [branchA.id, '编辑分支 A', 'A'],
      ])
  })
...
describe('deriveFlat', () => {
  it('flattens fork descendants under their root while unrelated roots stay newest-first', () => {
    ...
    expect(rows.map(row => row.id)).toEqual([sid('parent'), sid('tie-a'), sid('tie-b')])
    expect(rows[0]?.branches.map(row => row.id)).toEqual([sid('child')])
  })

  it('hides subagent-origin rows and nests ordinary forks', () => {
    const parent = summary('parent', 1)
    ...
    expect(rows[0]!.branches.map(row => row.id)).toEqual([fork.id])
  })
...
describe('conversationOrderUnits', () => {
  it('uses the newest member activity while anchoring the unit on its visible root', () => {
    const root = summary('root', 1)
    const child = forked('child', 'root', 30)
    const other = summary('other', 20)
    const sessions = list(root, child, other)

    expect(conversationOrderUnits([other.id, child.id, root.id], sessions)).toEqual([
      { id: other.id, sessionIds: [other.id], updatedAt: 20 },
      { id: root.id, sessionIds: [root.id, child.id], updatedAt: 30 },
    ])
  })
})
