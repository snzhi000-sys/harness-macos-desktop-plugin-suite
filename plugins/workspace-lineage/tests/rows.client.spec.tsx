...
describe('workspace browser rows', () => {
  it('uses one leading slot for a lineage root and shows branch activity without idle-row noise', () => {
    const root: SessionNode = {
      id: sid('root'), title: 'Root', blank: false, running: true,
      runningSubagentCount: 0, completed: false, updatedAt: 0,
      lineage: { role: 'root', sessionIds: [sid('root'), sid('branch')] },
      branches: [],
    }
    const view = render(<SessionNodeItem node={root} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} lineageExpanded t={t} />)
    const rootTitle = screen.getByText('Root')
    const toggle = screen.getByRole('button', { name: '收起会话分支' })
    expect(rootTitle.previousElementSibling).toBe(toggle)
    expect(toggle.querySelector('[data-state="ongoing"]')).not.toBeNull()

    const branch: SessionNode = {
      id: sid('branch'), title: '对话分支 A', blank: false, running: false,
      runningSubagentCount: 0, completed: false, updatedAt: 0,
      lineage: { role: 'branch', sessionIds: [sid('branch')], branchName: 'A' },
      branches: [],
    }
    view.rerender(<SessionNodeItem node={branch} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)
    expect(screen.getByRole('treeitem').querySelector('[data-state="done"]')).toBeNull()
    expect(screen.queryByText('空闲')).toBeNull()

    view.rerender(<SessionNodeItem node={{ ...branch, running: true }} currentId={undefined} now={0}
  ...
  it('uses only the selected row state for the current branch without changing its title', () => {
    const branch: SessionNode = {
      id: sid('selected-branch'), title: '对话分支 A', blank: false, running: false,
      runningSubagentCount: 0, completed: false, updatedAt: 0,
      lineage: { role: 'branch', sessionIds: [sid('selected-branch')], branchName: 'A' },
      branches: [],
    }
    render(<SessionNodeItem node={branch} currentId={branch.id} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)

    expect(screen.getByRole('treeitem').getAttribute('aria-selected')).toBe('true')
    expect(screen.getByText('对话分支 A')).toBeTruthy()
  ...
  it('session row menu opens without opening the session and dispatches rename, fork, and archive', () => {
    ...
    fireEvent.click(screen.getByRole('menuitem', { name: '重命名' }))
    expect(screen.queryByRole('menu')).toBeNull()
    expect(onRename).toHaveBeenCalledWith(node.id, 'One', false)
    expect(onOpen).not.toHaveBeenCalled()
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
  ...
  it('offers rename for a lineage branch and identifies it to the browser owner', () => {
    const onRename = vi.fn()
    const branch: SessionNode = {
      id: sid('branch'), title: '对话分支 A', blank: false, running: false,
      runningSubagentCount: 0, completed: false, updatedAt: 0,
    ...
    render(<SessionNodeItem node={branch} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={onRename} onFork={vi.fn()} onArchive={vi.fn()} t={t} />)

    fireEvent.click(screen.getByRole('button', { name: '会话“对话分支 A”的操作' }))
    fireEvent.click(screen.getByRole('menuitem', { name: '重命名' }))

    expect(onRename).toHaveBeenCalledWith(branch.id, '对话分支 A', true)
  })
