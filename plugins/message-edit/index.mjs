...
const HISTORICAL_VERSION_SUFFIX = "（编辑前版本）";
function baseVersionTitle(title) {
	let base = title.trim();
	while (base.endsWith(HISTORICAL_VERSION_SUFFIX)) base = base.slice(0, -HISTORICAL_VERSION_SUFFIX.length).trimEnd();
	return base;
}
async function labelVersionTitles(ctx, source, child) {
	const snapshot = ctx.sessionTitle.get(source.session);
	if (snapshot === void 0) return () => Promise.resolve();
	const originalTitle = snapshot.title;
	const baseTitle = baseVersionTitle(originalTitle);
	if (baseTitle.length === 0) return () => Promise.resolve();
	if (originalTitle !== baseTitle) {
		ctx.sessionTitle.rename(child.agent.session, baseTitle);
		await ctx.sessions.flush(child.agent.session);
	}
	const historicalTitle = `${baseTitle}${HISTORICAL_VERSION_SUFFIX}`;
	if (originalTitle === historicalTitle) return () => Promise.resolve();
	ctx.sessionTitle.rename(source.session, historicalTitle);
	try {
		await ctx.sessions.flush(source.session);
	} catch (error) {
		ctx.sessionTitle.rename(source.session, originalTitle);
		await ctx.sessions.flush(source.session);
		throw error;
	}
	return async () => {
...
function versionSeed(source, plan) {
	...
	// This event is plugin-only version metadata. Core history reconstruction
	// does not depend on it, so the persistence protocol requires the envelope
	// to be explicitly ignorable by a Harness build that does not know this
	// out-of-repo event type. Without the marker every restart refuses to load
	// the otherwise valid child session as "history unavailable".
...
async function runOperation(ctx, operation) {
	...
	return withSourceAgent(ctx, sourceId, async (source) => {
		...
		try {
			...
			const restoreSourceTitle = await labelVersionTitles(ctx, source, child);
			inverses.push(restoreSourceTitle);
			for (const message of plan.queuedUsers) child.agent.followup(message);
			lineageIndexPromise = void 0;
			inverses.length = 0;
			return {
				sessionId: childId,
...
async function buildLineageIndex(ctx) {
	const workspaceSessionIds = new Set(ctx.workspaceRegistry.list().flatMap((workspace) => workspace.sessionIds));
	const records = await ctx.sessionQuery.listSessions();
	const candidates = records.filter((record) => workspaceSessionIds.has(record.header.id) && record.header.parentSession !== void 0);
	const settled = await Promise.allSettled(candidates.map((record) => ctx.sessionQuery.readSession(record.header.id)));
	const versions = [];
	for (const [index, result] of settled.entries()) {
		if (result.status !== "fulfilled") continue;
		const record = result.value;
		const version = ownVersionEvent(record.session, record.events);
		if (version === void 0 || record.session.parentSession === void 0) continue;
		versions.push({
			sessionId: candidates[index].header.id,
			kind: "message-edit",
			parentSessionId: record.session.parentSession,
			operation: version.effect.operation,
			targetTurn: version.effect.targetTurn,
...
function lineageIndex(ctx) {
	lineageIndexPromise ??= buildLineageIndex(ctx).catch((error) => {
		lineageIndexPromise = void 0;
		throw error;
	});
...
async function handleRoute(ctx, request, response) {
	try {
		if (request.method === "GET") {
			const params = new URL(request.url ?? "/message-edit", "http://message-edit.local").searchParams;
			if (params.get("lineages") === "1") {
				respondJson(response, 200, await lineageIndex(ctx));
				return;
			}
			respondJson(response, 200, await timeline(ctx, sessionIdOf(params.get("sessionId"))));
			return;
		}
...
const messageEditProjectionSchema = {
	parse(value) {
		if (value === null) return null;
		if (typeof value !== "object" || value === null || Array.isArray(value)) throw new TypeError("messageEditVersion projection 不是对象");
		if (value.kind !== "message-edit" || typeof value.parentSessionId !== "string" || value.parentSessionId.length === 0) throw new TypeError("messageEditVersion projection 无效");
		if (value.operation !== "edit" && value.operation !== "reroll" && value.operation !== "retry") throw new TypeError("messageEditVersion operation 无效");
		if (!Number.isSafeInteger(value.targetTurn) || value.targetTurn < 0 || !Number.isFinite(value.createdAt)) throw new TypeError("messageEditVersion 数值无效");
		return value;
	}
};
function messageEditProjectionState(event) {
	if (event.type !== "message-edit/version") return void 0;
	const data = event.data;
	if (typeof data !== "object" || data === null) return void 0;
	if ("schemaVersion" in data) {
		if (data.schemaVersion !== 2 || data.inverse?.kind !== "restore-version") return void 0;
		return {
			kind: "message-edit",
			parentSessionId: data.inverse.sessionId,
			operation: data.effect.operation,
			targetTurn: data.effect.targetTurn,
...
function apply(ctx) {
	...
	ctx.inject(["sessionProjections"], (projectionCtx) => {
		projectionCtx.sessionProjections.register({
			key: "messageEditVersion",
			schema: messageEditProjectionSchema,
			init: () => null,
			apply: (state, event) => messageEditProjectionState(event) ?? state,
			view: (state) => state,
			stateVersion: 1
		});
	});
