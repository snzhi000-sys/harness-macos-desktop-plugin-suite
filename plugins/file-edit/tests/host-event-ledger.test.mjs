...
function harness(pluginImpl = plugin, sessionOverrides = {}, policyMode = 'workspace-write', pluginConfig = {}, services = {}) {
  ...
  const fs = {
    ...
    async writeText(target, content, _encoding, _version, policy) {
      if (policy?.mode === 'read-only') throw new Error('file access denied under read-only mode')
...
test('manual document saving in Read Only does not grant agent mutation permission', async () => {
  const h = harness(plugin, {}, 'read-only')
  const path = 'manual-readonly.md'
  writeFileSync(join(workspace, path), '# before\n')
  const diff = await h.invoke('getDiff', { sessionId: 'session-test', path })
  const result = await h.invoke('applyDocument', { sessionId: 'session-test', path, rev: diff.rev, content: '# manual save\n' })
  assert.equal(result.ok, true, JSON.stringify(result))
  assert.equal(readFileSync(join(workspace, path), 'utf8'), '# manual save\n')
  assert.equal(result.changed, false)
  const stale = await h.invoke('applyDocument', { sessionId: 'session-test', path, rev: diff.rev, content: 'stale' })
  assert.equal(stale.code, 'stale')
  const exec = { agent: { session: { id: 'session-test' } }, signal: new AbortController().signal }
  await assert.rejects(h.registered.get('file_delete').execute({ file_path: join(workspace, path) }, exec), /Read Only/)
  assert.equal(readFileSync(join(workspace, path), 'utf8'), '# manual save\n')
})

test('Workspace Write rejects external deletion including symlink parents before quarantine', async () => {
  const h = harness()
  const outside = join(temp, 'workspace-denied-delete')
  mkdirSync(outside)
  writeFileSync(join(outside, 'file.txt'), 'keep')
  const link = join(workspace, 'external-delete-link')
  symlinkSync(outside, link)
  const exec = { agent: { session: { id: 'session-test' } }, signal: new AbortController().signal }
  const before = state()
  for (const path of [outside, join(outside, 'file.txt'), join(link, 'file.txt')]) {
    await assert.rejects(h.registered.get('file_delete').execute({ file_path: path }, exec), /Workspace Write/)
    assert.equal(readFileSync(join(outside, 'file.txt'), 'utf8'), 'keep')
  }
  assert.deepEqual(state(), before)
  const full = harness(plugin, {}, 'danger-full-access')
  await full.registered.get('file_delete').execute({ file_path: outside }, exec)
  assert.equal(existsSync(outside), false)
})

test('manual Read Only saving rejects symlink escape and preserves pending agent work', async () => {
  const h = harness(plugin, {}, 'read-only')
  const outside = join(temp, 'manual-save-outside.md')
  writeFileSync(outside, 'external\n')
  const link = join(workspace, 'manual-save-link.md')
  symlinkSync(outside, link)
  const opened = await h.invoke('resolveOpenTarget', { sessionId: 'session-test', absolutePath: link })
  const diff = await h.invoke('getDiff', { sessionId: 'session-test', path: opened.id })
  const denied = await h.invoke('applyDocument', { sessionId: 'session-test', path: opened.id, rev: diff.rev, content: 'overwrite' })
  assert.equal(denied.ok, false)
  assert.equal(readFileSync(outside, 'utf8'), 'external\n')
  const path = 'manual-pending.md'
  writeFileSync(join(workspace, path), 'agent\n')
  await h.events.get('tools/result')({ name: 'write', agent: { session: { id: 'session-test' } } }, {
    isError: false, value: { path: join(workspace, path), operation: 'update', before: 'before\n', after: 'agent\n' },
  })
  const pending = await h.invoke('getDiff', { sessionId: 'session-test', path })
  const result = await h.invoke('applyDocument', { sessionId: 'session-test', path, rev: pending.rev, content: 'manual\n' })
  assert.equal(result.code, 'pending-review')
  assert.equal(readFileSync(join(workspace, path), 'utf8'), 'agent\n')
})
...
test('raw shell modes allow full access independently of snapshot availability', async () => {
...
test('full access missing roots dispatch once and retain coverage without injecting warnings', async () => {
  for (const enabled of [true, false]) {
    const sessionId = 'partial-missing-' + enabled
    const session = { id: sessionId, header: { id: sessionId, cwd: workspace } }
    const h = harness(plugin, { [sessionId]: session }, 'danger-full-access', { shellTransactionLifecycle: enabled })
    const exec = { name: 'bash', arguments: { command: 'synthetic write', audit_root: join(temp, 'not-created') }, agent: { session } }
    assert.equal((await h.events.get('tools/pre-execute')(exec, async () => ({ kind: 'allow' }))).kind, 'allow')
    let count = 0
    const output = await h.events.get('tools/execute')(exec, async () => {
      count++
      writeFileSync(join(temp, sessionId + '.txt'), 'outside workspace')
      return { isError: false, content: [{ type: 'text', text: 'done' }], value: { exitCode: 0 } }
    })
    assert.equal(count, 1)
    assert.equal(output.isError, false)
    assert.equal(h.events.has('tools/post-execute'), false)
    assert.deepEqual(output.content, [{ type: 'text', text: 'done' }])
    assert.equal(output.content.at(-1).text, 'done')
    assert.equal(stateForSession(sessionId).auditCoverage.kind, 'partial')
    const reboot = harness(plugin, { [sessionId]: session })
    const snapshot = await reboot.invoke('getModifiedSnapshot', { sessionId })
    assert.equal(snapshot.auditCoverage.kind, 'partial')
    assert.deepEqual(snapshot.files, [])
    assert.equal((await reboot.invoke('getModifiedSnapshot', { sessionId: 'another-session' })).auditCoverage, null)
  }
})

test('full access snapshot depth overflow does not block or duplicate execution and retains failure outcome', async () => {
  const root = join(temp, 'partial-depth')
  let deep = root
  for (let i = 0; i < 65; i++) deep = join(deep, 'd')
  mkdirSync(deep, { recursive: true })
  const sessionId = 'partial-overflow'
  const session = { id: sessionId, header: { id: sessionId, cwd: workspace } }
  const h = harness(plugin, { [sessionId]: session }, 'danger-full-access', { shellTransactionLifecycle: true })
  const exec = { name: 'bash', arguments: { command: 'synthetic', audit_root: root }, agent: { session } }
  let count = 0
  const result = await h.events.get('tools/execute')(exec, async () => {
    count++
    writeFileSync(join(temp, 'partial-overflow-out.txt'), 'written')
    return { isError: false, content: [], value: { exitCode: 17 } }
  })
  assert.equal(count, 1)
  assert.equal(result.value.exitCode, 17)
  const failure = new Error('original-command-failure')
  await assert.rejects(h.events.get('tools/execute')(exec, async () => {
    count++
    throw failure
  }), (error) => error === failure)
...
test('full access packaged Bash writes beyond its snapshot and keeps the exit marker', { skip: !process.env.DSH_TEST_RUNTIME_DIR }, async () => {
  const { pathToFileURL } = await import('node:url')
  const consumer = await import(pathToFileURL(join(process.env.DSH_TEST_RUNTIME_DIR, 'node_modules/@deepseek-ai/dsh-tool-bash/lib/index.js')).href)
  const root = mkdtempSync(join(temp, 'partial-native-'))
  const outside = mkdtempSync(join(temp, 'partial-native-outside-'))
  const sessionId = 'partial-native'
  const session = { id: sessionId, header: { id: sessionId, cwd: root } }
  const policy = { mode: 'danger-full-access', workspaceRoot: root }
  let definition
  let calls = 0
  consumer.apply({
    get: (name) => name === 'sandboxPolicy' ? { resolve: () => policy } : undefined,
    tools: { register: (def) => { definition = def; return () => {} } },
    effect: (fn) => fn(),
    systemPrompt: { section: () => () => {} },
    shellEnv: { collect: () => ({}) },
    shell: {
      sandboxMode: 'read-only', resolve: (spec) => spec,
      run: async (spec) => {
        assert.equal(spec.sandboxPolicy.mode, 'danger-full-access')
        calls++
        const stdout = execFileSync('/bin/bash', ['-c', spec.command], { cwd: root, timeout: 10_000, encoding: 'utf8' })
        return { exitCode: 0, stdout: { text: stdout, truncated: false }, stderr: { text: '', truncated: false } }
      },
    },
  }, { auditFullAccessWrites: true, enableRunInBackground: false })
  const h = harness(plugin, { [sessionId]: session }, () => policy, { shellTransactionLifecycle: true })
  const quote = (value) => "'" + value.replaceAll("'", "'\\''") + "'"
  const exec = {
    name: 'bash', callId: 'native-call', signal: new AbortController().signal, agent: { session },
    arguments: {
      command: 'printf inside > inside.txt; mkdir -p ' + quote(join(outside, 'new')) + '; printf outside > ' + quote(join(outside, 'new', 'out.txt')),
      description: 'synthetic outside writes', sandbox_permissions: 'danger-full-access', justification: 'already allowed',
    },
  }
  const result = await h.events.get('tools/execute')(exec, async () => {
    const value = await definition.execute(exec.arguments, exec)
    return { isError: false, value, content: [{ type: 'text', text: '[exit code: 0]' }] }
  })
  assert.equal(calls, 1)
  assert.equal(readFileSync(join(outside, 'new', 'out.txt'), 'utf8'), 'outside')
  assert.equal(stateForSession(sessionId).files['inside.txt'].cur.content, 'inside')
  assert.equal(h.events.has('tools/post-execute'), false)
  assert.deepEqual(result.content, [{ type: 'text', text: '[exit code: 0]' }])
})

test('full access settlement failure never reruns a successful command', async () => {
  const root = join(temp, 'partial-root-delete')
  mkdirSync(root)
  writeFileSync(join(root, 'old.txt'), 'before')
  const sessionId = 'partial-settlement'
  const session = { id: sessionId, header: { id: sessionId, cwd: workspace } }
  const h = harness(plugin, { [sessionId]: session }, 'danger-full-access', { shellTransactionLifecycle: true })
  let count = 0
  const result = await h.events.get('tools/execute')({
    name: 'bash', arguments: { command: 'synthetic root deletion', audit_root: root }, agent: { session },
  }, async () => {
    count++
    rmSync(root, { recursive: true })
    return { isError: false, content: [], value: { exitCode: 0 } }
  })
  assert.equal(count, 1)
...
test('file_delete quarantines external files with opaque review authority', async () => {
  const h = harness(plugin, {}, 'danger-full-access')
...
test('cold file browsing uses persisted headers without attaching history or scanning review targets', async () => {
  const sessionId = 'cold-browse-session'
  const target = join(workspace, 'cold-browse.txt')
  writeFileSync(target, 'cold content')
  let listed = 0
  const h = harness(plugin, {}, 'read-only', {}, { sessionPersistence: {
    list: async () => { listed++; return [{ id: sessionId, cwd: workspace }] },
    load: () => { throw new Error('must not load full history') },
  } })
  const opened = await h.invoke('resolveOpenTarget', { sessionId, path: 'cold-browse.txt', cwd: '/untrusted' })
  assert.equal(opened.ok, true, JSON.stringify(opened))
  assert.equal(opened.id, 'cold-browse.txt')
  const diff = await h.invoke('getDiff', { sessionId, path: opened.id })
  assert.equal(diff.ok, true, JSON.stringify(diff))
  assert.equal(h.listCalls, 0)
  assert.ok(listed > 0)
  const missing = await h.invoke('resolveOpenTarget', { sessionId: 'unknown-cold-session', absolutePath: target, cwd: workspace })
  assert.equal(missing.error, 'session-not-found')
})

test('migrated Stable profile permits real Bash writes and recoverable deletion', async (t) => {
  const workspace = mkdtempSync(join(temp, 'upgrade-workspace-'))
  const policy = () => ({ mode: 'workspace-write', workspaceRoot: workspace })
  const { pathToFileURL } = await import('node:url')
  const { createRequire } = await import('node:module')
  const { installBundledProfile } = await import(process.env.DSH_TEST_PROFILE_INSTALLER || '../../../desktop/src/profile-bootstrap.mjs')
  const resourcesPath = process.env.DSH_TEST_APP_RESOURCES || join(temp, 'upgrade-resources')
  if (!process.env.DSH_TEST_APP_RESOURCES) {
    mkdirSync(join(resourcesPath, 'profile-bootstrap'), { recursive: true })
    writeFileSync(join(resourcesPath, 'profile-bootstrap', 'profile-id'), '0123456789abcdef')
  }
  const userData = join(temp, 'upgrade-user-data')
  t.after(() => rmSync(userData, { recursive: true, force: true, maxRetries: 3, retryDelay: 100 }))
  t.after(() => rmSync(workspace, { recursive: true, force: true, maxRetries: 3, retryDelay: 100 }))
  const profile = join(userData, 'harness', 'profiles', 'web')
  mkdirSync(profile, { recursive: true })
  writeFileSync(join(profile, 'package.json'), '{"name":"legacy-user-profile"}')
  const original = '# preserve user composition\n- insert:\n    - id: dsh-file-edit\n      name: dsh-file-edit\n- id: personal-plugin\n  config: { setting: retained }\n'
  writeFileSync(join(profile, 'cordis.patch.yml'), original)
  const sessionId = 'session-profile-migration'
  const session = { id: sessionId, header: { id: sessionId, cwd: workspace } }
  const exec = { name: 'bash', callId: 'upgrade-write', arguments: { command: "printf upgraded > upgrade-write.txt" }, agent: { session } }
  const old = harness(plugin, { [sessionId]: session }, policy, {})
  const denied = await old.events.get('tools/pre-execute')(exec, async () => ({ kind: 'allow' }))
  assert.notEqual(denied.kind, 'allow')
  assert.equal(existsSync(join(workspace, 'upgrade-write.txt')), false)
  const options = { channel: 'stable', isPackaged: true, resourcesPath, userData, runtimeDir: process.env.DSH_TEST_RUNTIME_DIR }
  if (!process.env.DSH_TEST_APP_RESOURCES) options.extractArchive = async (_archive, destination) => {
    mkdirSync(join(destination, 'node_modules'), { recursive: true })
    writeFileSync(join(destination, 'package.json'), '{}')
  }
  assert.equal(await installBundledProfile(options), true)
  const require = createRequire(options.runtimeDir ? join(options.runtimeDir, 'package.json') : import.meta.url)
  const { parse } = require('yaml')
  const config = parse(readFileSync(join(profile, 'cordis.patch.yml'), 'utf8'))[0].insert[0].config
  assert.equal(config.shellTransactionLifecycle, true)
  assert.equal(readFileSync(join(profile, 'cordis.patch.yml.before-shell-transactions'), 'utf8'), original)
  assert.equal(await installBundledProfile(options), false)
  const migratedPlugin = process.env.DSH_TEST_APP_RESOURCES
    ? (await import(pathToFileURL(join(profile, 'node_modules/dsh-file-edit/host/index.mjs')).href)).default : plugin
  const h = harness(migratedPlugin, { [sessionId]: session }, policy, config)
  assert.equal((await h.events.get('tools/pre-execute')(exec, async () => ({ kind: 'allow' }))).kind, 'allow')
  const execute = async command => {
    exec.arguments.command = command
    exec.callId += '-next'
    return h.events.get('tools/execute')(exec, async () => {
      execFileSync('/bin/bash', ['-c', command], { cwd: workspace, timeout: 10000 })
      return { isError: false, value: { kind: 'foreground', exitCode: 0 } }
    })
  }
  await execute('printf upgraded > upgrade-write.txt')
  assert.equal(stateForSession(sessionId).files['upgrade-write.txt'].cur.content, 'upgraded')
  assert.equal((await h.invoke('rejectFile', { sessionId, path: 'upgrade-write.txt' })).ok, true)
  assert.equal(existsSync(join(workspace, 'upgrade-write.txt')), false)
  const directory = join(workspace, 'upgrade-delete')
  mkdirSync(join(directory, 'nested'), { recursive: true })
  writeFileSync(join(directory, 'nested', 'original.txt'), 'recover me')
  await execute('rm -rf -- upgrade-delete')
  assert.equal(existsSync(directory), false)
  const record = stateForSession(sessionId).files['upgrade-delete/nested/original.txt']
  assert.ok(record.deletion.deletionBatchId)
  assert.equal((await h.invoke('rejectDeletionBatch', { sessionId, deletionBatchId: record.deletion.deletionBatchId })).ok, true)
  assert.equal(readFileSync(join(directory, 'nested', 'original.txt'), 'utf8'), 'recover me')
})
