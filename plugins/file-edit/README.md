...
- **只读权限**：AI 调用 `file_delete` / `file_move` 时读取当前执行会话的实时 sandbox policy，在工具入口和磁盘修改前拒绝 `read-only`；`file_delete` 在 Workspace Write 下还校验执行会话的真实工作区，拒绝区外目标及符号链接父目录逃逸。拒绝不会创建删除隔离批次。用户在文件视图的整文和逐行保存不继承 AI 只读策略，仍限制在当前工作区并保留版本校验及待审核保护，不修改 AI 权限。
...
- **前台 Shell 审核**：Workspace Write 保持工作区修改前完整快照、容量限制和失败拒绝。Full Access 对可选 `audit_root` 或默认工作目录尝试快照，能结算的新增、修改与删除复用现有账本、隔离区和恢复；范围外写入、瞬时创建后删除、并发其他进程以及快照失败不保证记录或归因。权限不因审核失败被收窄，失败事务证据保留，不宣称全盘可恢复。
...
- Full Access 前台 Shell 在系统权限内执行，不以 `audit_root` 收窄写入。快照不可用、超限或结算失败不阻止命令或重放已经执行的命令；保留原始成功、非零退出或异常。会话持久保留 `auditCoverage.kind: partial`，不显示通用审核不完整文案或标签，也不向 Shell 结果附加这段提示。该记录不单独撑开空审核栏；实际操作错误及具体文件的恢复限制仍保留。移除提示不代表全盘审核完整。后台 Shell 与持久终端仍不开放。
